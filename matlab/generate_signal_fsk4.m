...
for i=1:(length(t))
    ...
    if(mod(i,(Fs/4)+1) == 0)
        rand_value = randi([0 3]);
        binary_signal(1,i) = rand_value;
            if(rand_value == 0)
                y(1,i) = cos(2*pi*t(1,i)*Fc1);
            elseif(rand_value == 1)
                y(1,i) = cos(2*pi*t(1,i)*Fc2);
            elseif(rand_value == 2)
                y(1,i) = cos(2*pi*t(1,i)*Fc3);            
            elseif(rand_value == 3)
                y(1,i) = cos(2*pi*t(1,i)*Fc4);            
            end
...
while j ~= num_samples
    ...
    for i=(j*Fs/4)+1:(j+1)*(Fs/4)
        ...
        elseif(binary_signal(1,i) == 1)
            sum_frequency = sum_frequency + (1/(Fs/4))*Fc2;
        elseif(binary_signal(1,i) == 2)
            sum_frequency = sum_frequency + (1/(Fs/4))*Fc3;
        elseif(binary_signal(1,i) == 3)
            sum_frequency = sum_frequency + (1/(Fs/4))*Fc4;            
        end
    end

    for i=(j*Fs/4)+1:(j+1)*(Fs/4)
        if(binary_signal(1,i) == 0)
            normalized_frequency(1,i) = (((Fc1-sum_frequency)/(Rb/2))*(Rb/2) ) - Baud_rate; 
        elseif(binary_signal(1,i) == 1)
            normalized_frequency(1,i) = (((Fc2-sum_frequency)/(Rb/2))*(Rb/2) ) - Baud_rate; 
        elseif(binary_signal(1,i) == 2)
            normalized_frequency(1,i) = (((Fc3-sum_frequency)/(Rb/2))*(Rb/2) ) - Baud_rate; 
        elseif(binary_signal(1,i) == 3)
            normalized_frequency(1,i) = (((Fc4-sum_frequency)/(Rb/2))*(Rb/2) ) - Baud_rate;        
        end            
    end
    ...
    feature_matrix(j+1,6) = 4;
    

    j = j + 1;
end

writematrix(feature_matrix,'fsk4_test_data.xls')
