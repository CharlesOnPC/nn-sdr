...
for i=1:(length(t))
    ...
    if(mod(i,(Fs/4)+1) == 0)
        ...
        binary_signal(1,i) = rand_value;
            if(rand_value == 0)
                y(1,i) = cos((2*pi*t(1,i)*Fc1)+ pi/4);
            elseif(rand_value == 1)
                y(1,i) = cos((2*pi*t(1,i)*Fc1)+ 3*pi/4);
            elseif(rand_value == 2)
                y(1,i) = cos((2*pi*t(1,i)*Fc1)+ 5*pi/4);
            elseif(rand_value == 3)
                y(1,i) = cos((2*pi*t(1,i)*Fc1)+ 7*pi/4);                
            end
...
while j ~= num_samples
    ...
    feature_matrix(j+1,6) = 6;

    j = j + 1;
end

writematrix(feature_matrix,'psk4_test_data.xls')
