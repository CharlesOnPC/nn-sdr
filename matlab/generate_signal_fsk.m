...
while j ~= num_samples
    ...
    for i=(j*Fs/4)+1:(j+1)*(Fs/4)
        if(binary_signal(1,i) == 0)
            centered_non_inear_instantaneous_phase(1,i) = unwrapped_phase(1,i) - ((2*pi*(1)*i)/Fs);
        else
            centered_non_inear_instantaneous_phase(1,i) = unwrapped_phase(1,i) - ((2*pi*(8)*i)/Fs);
        end
    end
    ...
    for i=(j*Fs/4)+1:(j+1)*(Fs/4)
        if(binary_signal(1,i) == 0)
            sum_frequency = sum_frequency + (1/(Fs/4))*1;
        else
            sum_frequency = sum_frequency + (1/(Fs/4))*8;
        end
    end
    ...
    for i=(j*Fs/4)+1:(j+1)*(Fs/4)
        if(binary_signal(1,i) == 0)
            ...
            normalized_frequency(1,i) = (((Fc2-sum_frequency)/Rb)*Rb ) - Baud_rate; 
        end    
    end
    ...
    feature_matrix(j+1,6) = 3;

    j = j + 1;
end

writematrix(feature_matrix,'fsk2_test_data.xls')
