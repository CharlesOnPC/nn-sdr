...
for i=1:(length(t))
    ...
    if(mod(i,(Fs/4)+1) == 0)
        ...
        binary_signal(1,i) = rand_value;
            if(rand_value == 0)
                y(1,i) = 1*cos(2*pi*t(1,i)*Fc1);
            elseif(rand_value == 1)
                y(1,i) = 2*cos(2*pi*t(1,i)*Fc1);
            elseif(rand_value == 2)
                y(1,i) = 3*cos(2*pi*t(1,i)*Fc1);
            elseif(rand_value == 3)
                y(1,i) = 4*cos(2*pi*t(1,i)*Fc1);                
            end
...
while j ~= num_samples
    ...
    feature_matrix(j+1,6) = 2;

    j = j + 1;
end

writematrix(feature_matrix,'ask4_test_data.xls')
